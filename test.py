...
class FlaskTest(unittest.TestCase):
    ...
    def test_get_thread(self):
        response = self.tester.get('api/chaddit/c/threads',
                                   headers={
                                       'post_id': self.test_post_id,
                                       'thread_id': self.test_thread_id,
    ...
    def test_get_user(self):
        response = self.tester.get(f"api/chaddit/c/user/${self.test_user_id}", headers={
            'api_token': self.admin_token
        })
        statusCode = response.status_code
        if statusCode != 200:
    ...
    def test_successful_register(self):
        response = self.tester.post("api/chaddit/c/register", headers={
    ...
    def test_get_message_for_chat(self):
        response = self.tester.get(f"api/chaddit/c/messages/${self.test_message_id}")
        statusCode = response.status_code
        if statusCode != 200:
            print(response.data)
        self.assertEqual(statusCode, 200)
    ...
    def test_on_decode_bad_token(self):
        ...
        self.assertEqual(newToken['error'],
                         {'message': 'Invalid admin_token, please try again with a new admin_token.'})
    ...
    def test_on_bad_update_user(self):
        response = self.tester.patch('api/chaddit/c/user/-1', headers={
            'api_token': self.admin_token,
        })
        statusCode = response.status_code
        if statusCode != 200:
